...
as $$
  select
    ...
    follower_profile.created_at::timestamptz
  ...
  order by follower_profile.created_at desc nulls last, follower_profile.user_id
...
as $$
  select
    ...
    following_profile.created_at::timestamptz
  from public.follows follow_rows
  ...
  where follow_rows.follower_id = p_user_id
    and auth.uid() is not null
  order by following_profile.created_at desc nulls last, following_profile.user_id
  limit least(greatest(coalesce(p_limit, 50), 0), 100)
  offset greatest(coalesce(p_offset, 0), 0);
$$;
