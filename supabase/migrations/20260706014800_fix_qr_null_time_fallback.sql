-- Migration: Fix QR fallback when event_start_time is null (AG-18B)
-- Redefine verify_and_check_in_participant to use exact timezone calculation when event_start_time is null.
...
BEGIN
  ...
  -- Precise calculation of event start time
  ...
  ELSE
    v_event_start := ((v_event.event_date AT TIME ZONE 'UTC') AT TIME ZONE 'Europe/Istanbul');
  END IF;
